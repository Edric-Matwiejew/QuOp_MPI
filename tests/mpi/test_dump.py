...
def _run_qaoa(comm, system_size=4, depth=1):
...
class TestDumpEnabled:
    def test_dump_creates_files_in_cwd(self, mpi_comm, tmp_path, monkeypatch):
...
class TestDumpToDirectory:
    def test_dump_to_custom_directory(self, mpi_comm, tmp_path, monkeypatch):
...
class TestDumpDisabled:
    def test_no_dump_when_unset(self, mpi_comm, tmp_path, monkeypatch):
