    ...
    def test_set_log_adds_csv_extension(self, mpi_comm, simple_oracle):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if mpi_comm.Get_rank() == 0:
                ...
                assert not os.path.exists(log_path) or os.path.isdir(log_path) or log_path.endswith('.csv')
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if mpi_comm.Get_rank() == 0:
                ...
                assert label in content, \
                    f"Label '{label}' not found in log file"
    ...
    def test_log_contains_depth(self, mpi_comm, simple_oracle):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if mpi_comm.Get_rank() == 0:
                ...
                assert str(depth) in content, \
                    f"Depth {depth} not found in log file"
