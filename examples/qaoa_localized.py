...
for qubits in range(2,4):
    ...
    qaoa.set_initial_state(name = "localized")
    qaoa.log_success("qaoa_localized","localized",action="a")
    qaoa.set_qualities(qw.qualities.random_floats)
    qaoa.benchmark(range(1,6), 5, param_func = x0, qual_func = qw.qualities.random_floats, filename = "qaoa_localized",label = "qaoa_" + str(qubits))
