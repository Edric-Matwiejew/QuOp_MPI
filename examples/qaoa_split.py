...
for qubits in range(2,4):
    ...
    qaoa.set_initial_state(name = "split")
    qaoa.log_success("qaoa_split","split",action="a")
    qaoa.set_qualities(qw.qualities.random_floats)
    qaoa.benchmark(range(1,6), 5, param_func = x0, qual_func = qw.qualities.random_floats, filename = "qaoa_split",label = "qaoa_" + str(qubits))
