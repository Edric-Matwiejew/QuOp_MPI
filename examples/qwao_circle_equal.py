...
for qubits in range(2,4):
    ...
    qwao.log_success("qwao_circle_equal","equal",action="a")
    qwao.set_graph(qw.graph_array.circle(qwao.size))
    qwao.plan()
    qwao.benchmark(range(1,6), 5, param_func = x0, qual_func = qw.qualities.random_floats, filename = "qwao_circle_equal",label = "qwao_" + str(qubits))
    qwao.destroy_plan()
