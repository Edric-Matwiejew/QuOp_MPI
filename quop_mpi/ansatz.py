...
# Each set_*() ORs the relevant flags with cascades.  setup() processes
# them in dependency order and clears each flag after completion.
...
class Ansatz(Sampling, Logging, Communicator, Jacobian, Benchmark, Bindable):
    ...
    * :term:`Sampling Function` (:meth:`~quop_mpi.ansatz.set_sampling`)
    ...
    def __init__(self, system_size: int, MPI_communicator: Intracomm = MPI.COMM_WORLD) -> None:  # noqa: N803
        ...
        self._is_active = True  # False if rank was excluded by negotiate

        # -- Backward-compat boolean flags -----------------------
        # Subsystems not yet converted to dirty flags (sampling, logging,
        # objective) still use boolean gates.
    ...
    def __pre(self) -> None:
        ...
        Dirty-flag driven: only regenerates subsystems whose flags are
        set.  Backward-compat booleans are kept in sync so that
        subsystems not yet converted (sampling, logging, objective,
        var_map) continue to work.
        """
        ...
        # objective stays boolean-driven (not in _Dirty model)
        ...
        # sampling and logging stay boolean-driven (not in _Dirty model)
    ...
    def setup(self) -> None:
        ...
        if self._dirty & (_Dirty.NEGOTIATION | _Dirty.WORKER_SPLIT):
            ...
            # Record active status -- excluded ranks proceed normally;
            # work is skipped via in_subcomm() guards, NOT by parking.
            self._is_active = self.subcomms.in_subcomm()
