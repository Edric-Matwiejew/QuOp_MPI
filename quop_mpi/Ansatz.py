...
class ansatz(object):
    ...
    def __init__(self, system_size, MPI_communicator, parallel = "global")
        ...
        self.initial_state_signatures = {
                'serial':['system_size']
                'mpi':['system_size', 'local_i', 'local_i_offset']
                }

        self.initial_state_returns = {
                'initial state':[self.empty(2, np.complex128)]
                }

        self.observable_map_signatures = {
                'observable map':['observables']
                }

        self.observable_map_returns = {
                'observable map':[np.empty(2, np.complex128)]
                }

    def set_unitaries(self, unitaries):
        self.unitaries = unitaries

    def set_observables(self, index):

        if self.unitaries[index].unitary_type == "diagonal"
            self.observables = self.unitaries[index].operator
        else:
    ...
    def set_optimiser(self, optimiser, optimiser_args = {}, optimiser_log = None):
        ...
        if optimiser == 'scipy':
            ...
            from quop_mpi._nlopt_wrap import minimize as nlopt_minimize
    ...
    def pre(self):
        ...
        if self.colours[self.COMM.Get_rank()] != -1:
            ...
            for unitary in enumerate(self.unitaries):
                if unitary.planner:
                    planner = unitary
            ...
            if self.initial_state_input is None:
                from initial_state import equal
                self.set_initial_state(equal)

            self.initial_state = self.initial_state_function.function()

            for unitary in unitaries:
                unitary.initial_state = self.initial_state

            self.param_map = np.zeros(len(self.unitaries), int)
            for i, unitary in enumerate(self.unitaries):
                self.param_map[i] = unitary.n_params
    ...
    def evolve_state(self, x):
        ...
        if self.colours[self.COMM.Get_rank()] != -1:
            ...
            for depth, params in enumerate(params_split):
                ...
                for param_group, unitary in zip(params, self.unitaries):

                    if unitary.operator_function.n_variational_parameters > 0:

                        operator_parameters = param_group[:-1]
                        evolution_parameter = param_group[0]

                        unitary.operator_function(operator_parameters)
                        unitary.update()

                    else:
                        evolution_parameter = param_group

                    if not self._is_zero(evolution_parameter):
    ...
    def execute(self, post = True, **kwargs):
        """
        ...
        if not self.benchmarking:
            self.variational_parameters = self.__gen_initial_params(**kwargs)
        ...
        if self.colours[self.COMM.Get_rank()] != -1:
            ...
            if self.colours[self.COMM.Get_rank()] == 0:
                ...
                if self.COMM_OPT.Get_rank() == 0:
                    ...
                    while not self.stop:
                        self._objective(self.variational_parameters)
            ...
            for depth in ansatz_depths:
                ...
                for i  in range(1, repeats + 1):
                    ...
                    if (not param_persist) or first:
                        ...
                        self.variational_parameters = self.__gen_initial_params(seed = i + itter)

                    else:

                        new_parameters = self.__gen_initial_params(seed = i + itter)
    ...
    def __gen_initial_params(self, **kwargs):

        params = np.empty(self.total_params, np.float64)

        if self.COMM_OPT.Get_rank() == 0:

            param_groups = np.array_split(params, self.param_map)

            for param, unitary in zip(param_groups, self.unitaries):
                param[:] = unitary.get_initial_params(**kwargs)

        self.COMM_OPT.Bcast([params, MPI.DOUBLE], params, 0)
