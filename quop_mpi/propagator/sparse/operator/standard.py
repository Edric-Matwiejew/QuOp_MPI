...
def hypercube(
    ...
    if system_size % 2 != 0:
        raise RuntimeError(
            "Specified system_size not compatible with hypercube mixer."
            " Requires system_size = 2**n for integer n."
        )
...
def serial(
    ...
    **kwargs,
    ...
    if MPI_COMM.Get_rank() == 0:
        ...
        W = function(*input_args, **kwargs)  # noqa: N806

        if not isinstance(W, list):
            raise TypeError("User supplied function must return a list of scipy CSR matrices.")
...
def qmoa_mixer(local_i, local_i_offset, Ns, Gs):  # noqa: N803
    """Generate a CSR mixing operator for multivariable quantum optimisation."""
    from quop_mpi._lib.csr_generators import csr_generators
    ...
    G_ptrs = np.empty(n_dim, dtype=np.int64)  # noqa: N806

    for i, G in enumerate(Gs):  # noqa: N806
    ...
    W_row_starts, W_col_indexes, W_values = csr_generators.qmoa_mixer(  # noqa: N806
        local_i,
        local_i_offset,
        n_grid_points,
