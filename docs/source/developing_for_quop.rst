...
A :mod:`~quop_mpi.propagator` module should also implement an ``operator`` submodule containing :term:`Operator Functions <Operator Function>` that are compatible with the implemented :meth:`~quop_mpi.Unitary.propagate` method.